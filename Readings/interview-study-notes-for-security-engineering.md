...
These notes were created while the author was preparing for security engineering interviews and later became useful to many people trying to enter or grow in security. The main advice is to study based on the role, build a strong list of terms and technologies to understand, practice coding seriously, and keep improving weaker areas instead of trying to be an expert in everything at once. The later update emphasizes that coding is often the biggest blocker in security engineering interviews, so it is worth giving yourself enough time to practice, get help, and try again if needed.

Reference: [gracenolan/Notes](https://github.com/gracenolan/Notes/)
